...
public class HuffmanListFundTest {
	...
	//tests byte andint constructor, the only other constructor left to test. Does the array out of order.
	@Test
	public void testByteAndIntegerArrayConstructor() {
		try	{
			HuffmanList list = new HuffmanList(new byte[]{(byte)'b', (byte)'a', (byte)'c'}, new int[] {2, 1, 3});
			Iterator<HuffmanNode> it = list.iterator();
			assertEquals(
				"This method provides an iterator of the list. Check that the iterator has values.",
				true, it.hasNext());
	...
	//tests byte and int constructor, the only other constructor left to test. Does the array in order.
	@Test
	public void testByteAndIntegerArrayConstructor2() {
		try	{
			HuffmanList list = new HuffmanList(new byte[]{(byte)'a', (byte)'b', (byte)'c'}, new int[] {1, 2, 3});
			Iterator<HuffmanNode> it = list.iterator();
			assertEquals(
				"This method provides an iterator of the list. Check that the iterator has values.",
				true, it.hasNext());
